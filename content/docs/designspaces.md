...
When you define those axis values, you make a request for a discrete design space location for use with `h1` heading text.  You can think of this as defining a "static instance" within the dynamic design space in a font.  You can request a different area of the space by setting different axis tag values. As demonstrated in the CodePen example, it is possible to use multiple design space locations to format different text fields in the document.  This is akin to having a knob for each axis to dial in the exact design that you want from the space.  GUI graphics editors, text editors, and other applications that support variable fonts expose axis settings in a similar fashion.  You'll commonly be presented with the axis tag name and a text box or slider to select the axis value.
