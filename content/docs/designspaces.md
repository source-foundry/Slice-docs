...
Static instances are one type of output.  Slice provides a list of available axis tags (and axis names when you hover over the tag) as well as the default, min, and max axis values.  Use these data to define a numeric value within the design space for every axis in the variable font, and Slice creates a new static instance font at the location you specify.
